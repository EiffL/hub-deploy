...
locals {
  ...
  user_ksa       = "${local.name}-user"
  user_principal = "principal://iam.googleapis.com/projects/${data.google_project.current.number}/locations/global/workloadIdentityPools/${data.google_client_config.provider.project}.svc.id.goog/subject/ns/${local.name}/sa/${local.user_ksa}"
}
